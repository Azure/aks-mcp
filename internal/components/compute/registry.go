...
func RegisterCollectAKSNodeLogsTool() mcp.Tool {
	return mcp.NewTool(
		...
		mcp.WithDescription("Collect kubelet, containerd, kernel (dmesg), or syslog logs from AKS node using VMSS run command. Useful for debugging node-level issues. Supports filtering by time range and log level. IMPORTANT: Only ONE run command can execute at a time per VMSS instance - wait for completion before running another command on the same instance."),
